...
class Game : RComponent<GameProps, GameState>() {
    ...
    private fun joinGame(gameId: String, playerName: String) {
        ...
        val socket = WebSocket("$protocol//${window.location.host}/play/v1/events?game-id=$gameId")
    ...
    private fun newGame(playerName: String) {
        val requestUrl = "${window.location.protocol}//${window.location.host}/play/v1/new-game"
